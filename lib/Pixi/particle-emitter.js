...
let GetTextureFromString = PIXI.Texture.from;
/**
...
class Emitter {
    /**
     * @param {PIXI.ParticleContainer} container The container to add the particles to.
    ...
    constructor(container, config) {
        ...
        this._prevEmitterPos = new PIXI.Point();
        ...
        /** @type {PIXI.ParticleContainer} */ this.parent = container;
        if (config) {
            this.init(config);
        }
        // save often used functions on the instance instead of the prototype for better speed
    ...
    initParticles(first) {
        ...
        while (next) {
            ...
            if (!next.config.velocity) {
                next.config.velocity = new PIXI.Point(speed, 0);
            }
            else {
...
class RandomAnimatedTextureBehavior {
    ...
    updateParticle(particle, deltaSec) {
        ...
        particle.texture = anim.textures[frame] || anim.textures[anim.textures.length - 1] || PIXI.Texture.EMPTY;
    }
}
    ...
    initParticles(first) {
        ...
        while (next) {
            ...
            if (!next.config.initPosition) {
                next.config.initPosition = new PIXI.Point(next.x, next.y);
            }
            else {
...
class SpeedBehavior {
    ...
    initParticles(first) {
        ...
        while (next) {
            ...
            next.config.speedMult = mult;
            if (!next.config.velocity) {
                next.config.velocity = new PIXI.Point(this.list.first.value * mult, 0);
            }
            else {
 ...
 */
class LinkedListContainer extends PIXI.Container {
    constructor() {
        super(...arguments);
    ...
    getLocalBounds(rect, skipChildrenUpdate = false) {
        ...
        const result = PIXI.DisplayObject.prototype.getLocalBounds.call(this, rect);
...
// export
export { Emitter, LinkedListContainer, Particle, PropertyList, PropertyNode, upgradeConfig, ParticleUtils, index$1 as behaviors }
